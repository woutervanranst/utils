...
public class ConcurrentConsumingTaskCollectionTests
{
    ...
    {
        // Arrange
        var taskQueue = new TaskCollector<string>();
        ...
        await foreach (var result in taskQueue.GetCompletedTasks())
        {
...
public class TaskCollectorTests
{
    [Fact]
    public async Task ShouldYieldTasksInCompletionOrder()
    {
        var collector = new TaskCollector<int>();
        ...
        {
            await foreach (var task in collector.GetCompletedTasks())
            {
    ...
    {
        var collector = new TaskCollector<int>();
        var enumerationTask = collector.GetCompletedTasks().GetAsyncEnumerator(); //.MoveNextAsync();
